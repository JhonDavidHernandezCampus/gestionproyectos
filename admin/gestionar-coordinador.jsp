...
<nav class="navbar navbar-expand-lg navbar-light bg-light">
    <div class="container">
        <a class="navbar-brand" href="#">Gestionar Coordinadores</a>
...
<div class="container mt-5">
    <div class="d-flex">
    <h3 class="me-4">Listado de Coordinadores</h3>
    <div class="me-2"> <a href="./agregarCoo.jsp" class="btn btn-sm btn-success 
    p-2">Agregar Coordinador</a></div>
    </div>
    ...
    <table class="table table-striped mt-3">
        ...
        <tbody>
            <sql:query var="coordinadores" dataSource="${gestion_proyectos}">
                SELECT *
                FROM usuario
                WHERE rol_usu = 'coordinador'
            </sql:query>
            <c:forEach var="coordinador" items="${coordinadores.rows}">
                <tr>
                    <td>${coordinador.id_usu}</td>
                    <td>${coordinador.nombres_usu}</td>
                    <td>${coordinador.apellidos_usu}</td>
                    <td>${coordinador.direccion_usu}</td>
                    <td>${coordinador.telefono_usu}</td>
                    <td>${coordinador.email_usu}</td>
                    <td>
                        <a href="./editarCoo.jsp?id=${coordinador.id_usu}" class="btn btn-sm btn-primary">Editar</a>
                        <a href="./eliminarUsu.jsp?id=${coordinador.id_usu}" class="btn btn-sm btn-danger">Eliminar</a>
                    </td>
                </tr>
            </c:forEach>
        </tbody>
