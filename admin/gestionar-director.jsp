...
<head>
    ...
    <title>Gestionar Directores</title>
...
<nav class="navbar navbar-expand-lg navbar-light bg-light">
    <div class="container">
        <a class="navbar-brand" href="#">Gestionar Directores</a>
...
<div class="container mt-5">
    <div class="d-flex">
    <h3 class="me-4">Listado de Directores</h3>
    <div class="me-2"> <a href="./agregarDir.jsp" class="btn btn-sm btn-success 
    p-2">Agregar Director</a></div>
    </div>
    ...
    <table class="table table-striped mt-3">
        ...
        <tbody>
            <sql:query var="directores" dataSource="${gestion_proyectos}">
                SELECT *
                FROM usuario
                WHERE rol_usu = 'director'
            </sql:query>
            <c:forEach var="director" items="${directores.rows}">
                <tr>
                    <td>${director.id_usu}</td>
                    <td>${director.nombres_usu}</td>
                    <td>${director.apellidos_usu}</td>
                    <td>${director.direccion_usu}</td>
                    <td>${director.telefono_usu}</td>
                    <td>${director.email_usu}</td>
                    <td>
                        <a href="./editarDir.jsp?id=${director.id_usu}" class="btn btn-sm btn-primary">Editar</a>
                        <a href="./eliminarUsu.jsp?id=${director.id_usu}" class="btn btn-sm btn-danger">Eliminar</a>
                    </td>
                </tr>
            </c:forEach>
        </tbody>
