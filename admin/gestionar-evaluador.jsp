...
<nav class="navbar navbar-expand-lg navbar-light bg-light">
    <div class="container">
        <a class="navbar-brand" href="#">Gestionar Evaluadores</a>
...
<div class="container mt-5">
    <div class="d-flex">
    <h3 class="me-4">Listado de Evaluadores</h3>
    <div class="me-2"> <a href="./agregarEva.jsp" class="btn btn-sm btn-success 
    p-2">Agregar Evaluador</a></div>
    </div>
    ...
    <table class="table table-striped mt-3">
        ...
        <tbody>
            <sql:query var="evaluadores" dataSource="${gestion_proyectos}">
                SELECT *
                FROM usuario
                WHERE rol_usu = 'evaluador'
            </sql:query>
            <c:forEach var="evaluador" items="${evaluadores.rows}">
                <tr>
                    <td>${evaluador.id_usu}</td>
                    <td>${evaluador.nombres_usu}</td>
                    <td>${evaluador.apellidos_usu}</td>
                    <td>${evaluador.direccion_usu}</td>
                    <td>${evaluador.telefono_usu}</td>
                    <td>${evaluador.email_usu}</td>
                    <td>
                        <a href="./editarEva.jsp?id=${evaluador.id_usu}" class="btn btn-sm btn-primary">Editar</a>
                        <a href="./eliminarUsu.jsp?id=${evaluador.id_usu}" class="btn btn-sm btn-danger">Eliminar</a>
                    </td>
                </tr>
            </c:forEach>
        </tbody>
