...
<sql:update dataSource="${gestion_proyectos}">
    INSERT INTO usuario (id_usu, contrasena_usu, nombres_usu, apellidos_usu, direccion_usu, telefono_usu, email_usu, rol_usu) 
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
 ...
 <c:set var="destination" value="${param.destinatination}" />
    <c:choose>
        <c:when test="${destination == 'coordinador'}">
            <c:redirect url="gestionar-coordinador.jsp" />
        </c:when>
        <c:when test="${destination == 'director'}">
            <c:redirect url="gestionar-director.jsp" />
        </c:when>
        <c:when test="${destination == 'estudiante'}">
            <c:redirect url="gestionar-estudiante.jsp" />
        </c:when>
        <c:when test="${destination == 'evaluador'}">
            <c:redirect url="gestionar-evaluador.jsp" />
        </c:when>
        <c:otherwise>
